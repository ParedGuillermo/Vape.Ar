// src/components/VapeCommunity/ReseñasDeProductos.jsx
import React, { useEffect, useState } from "react";
import { supabase } from "../../supabaseClient"; // Asegúrate de que el path sea correcto
import { AuthProvider } from "../../context/AuthContext"; // Corregir la ruta
import { CartProvider } from "../../components/CartContext"; // También corregir la ruta si es necesario

export default function ReseñasDeProductos() {
  const [entradas, setEntradas] = useState([]);
  const categoria = "Reseñas de Productos";
  ...
  const fetchEntradas = async () => {
    ...
    else console.log("Error fetching entries:", error);
  };
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-b from-[#1a1a2e] via-[#16213e] to-[#0f3460] text-gray-300">
      ...
      <div className="space-y-6">
        {entradas.length === 0 ? (
          <p className="text-center text-[#c9d1d9]">No hay reseñas publicadas en esta categoría.</p>
        ) : (
