...
export default function GuiasYTutoriales() {
  ...
  useEffect(() => {
    const fetchEntradas = async () => {
      ...
      const { data, error } = await supabase
        .from("entradas_blog")
        .select("*")
        .eq("categoria", "guias") // O "tutoriales", según cómo la cargues
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-b from-[#1a1a2e] via-[#16213e] to-[#0f3460] text-gray-300">
      ...
      {loading ? (
        <p className="text-center text-gray-400">Cargando guías...</p>
      ) : entradas.length === 0 ? (
        <p className="text-center text-gray-400">No hay contenido disponible.</p>
      ) : (
